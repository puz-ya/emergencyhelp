...
public /* static */ class SettingsFragmentOther
        extends AutoSummaryPreferenceFragment {

    final String[] mKeys = {"alarm_time", "alarm_sensitive"};

    Context mSettingsActivity = null;
    private static final String LOG_TAG = "e.y/S...entOther";
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        Preference myPrefAlarmTime = findPreference(mKeys[0]);
        String mUserName = sql.getSettingsValue(mKeys[0]);
        myPrefAlarmTime.setSummary(mUserName);
        myPrefAlarmTime.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
        ...
        Preference myPrefAlarmSensit = findPreference(mKeys[1]);
        String mUserComment = sql.getSettingsValue(mKeys[1]);
        myPrefAlarmSensit.setSummary(mUserComment);
        myPrefAlarmSensit.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                EditTextPreference etp = (EditTextPreference) preference;
                String newHostValue = newValue.toString();
